...
import { supabase, type UserBadge, type Badge, type MissionHistory } from '../lib/supabase';
...
export default function HallOfFame() {
  const { userBadges, weeklyMissions, currentWeek } = useLoaderData<typeof loader>();
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-3xl shadow-soft p-6 mb-8">
        <h2 className="text-xl font-semibold mb-4 text-primary-600">내 배지 컬렉션</h2>
        
        {userBadges.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-gray-500">아직 획득한 배지가 없어요. 도전과제를 달성해보세요!</p>
          </div>
        ) : (
          <div className="honeycomb-grid">
            {userBadges.map((userBadge, index) => (
              <div key={userBadge.id} className="honeycomb-cell" style={{ 
                '--i': index 
      ...
      <div className="bg-white rounded-3xl shadow-soft p-6">
        <h2 className="text-xl font-semibold mb-4 text-primary-600">주간 미션 달성 기록</h2>
        
        <div className="space-y-6">
          {Object.keys(weeklyMissions).length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-500">아직 완료한 미션이 없어요. 오늘의 미션을 완료해보세요!</p>
            </div>
          ) : (
            Object.entries(weeklyMissions).map(([week, missions]) => (
              <div key={week} className="border border-gray-200 rounded-2xl p-4">
                <h3 className="text-lg font-medium mb-3 text-gray-800">
                  {currentWeek === Number(week) ? '이번 주' : `${week}주차`} (완료 미션: {missions.length}개)
                </h3>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {missions.map(history => (
                    <div key={history.id} className="bg-gray-50 rounded-xl p-3 flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          {new Date(history.completed_at).toLocaleDateString('ko-KR', { 
                            month: 'long', 
                            day: 'numeric', 
                            hour: '2-digit', 
                            minute: '2-digit' 
