...
import { supabase, type Mission, type Challenge } from "../lib/server";
...
export async function loader() {
  ...
  // 도전 과제 목록
  const { data: challenges, error: challengesError } = await supabase.from(
    "challenges"
  ).select(`
      *,
      badge:badges(*)
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-3xl shadow-soft p-6 mb-8">
        ...
        <p className="text-sm text-gray-500 mb-6">
          각 요일별로 3~10개 사이의 미션을 설정할 수 있어요.
        </p>
        ...
        {challenges.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {challenges.map((challenge) => (
              <div
                key={challenge.id}
                className="border border-gray-200 rounded-2xl p-4 flex items-center"
              >
